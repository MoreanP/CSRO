...
class SimpleReplayBuffer_z(ReplayBuffer):
    def __init__(
            self, max_replay_buffer_size, observation_dim, action_dim, goal_radius, latent_size
    ):
        ...
        self._last_z = np.zeros((max_replay_buffer_size, latent_size))
        self.goal_radius = goal_radius
        self.clear()

    def add_sample(self, observation, action, reward, terminal,
                   next_observation, last_z, **kwargs):
        self._observations[self._top] = observation
        self._actions[self._top] = action
        self._rewards[self._top] = reward
        self._terminals[self._top] = terminal
        self._next_obs[self._top] = next_observation
        self._last_z[self._top] = last_z
    ...
    def sample_data_z(self, indices):
        return dict(
            observations=self._observations[indices],
            actions=self._actions[indices],
            rewards=self._rewards[indices],
    ...
    def random_batch_z(self, batch_size):
        ''' batch of unordered transitions '''
        assert self._size > 0
        indices = np.random.randint(0, self._size, batch_size)
        return self.sample_data_z(indices)
    ...
    def random_sequence_z(self, batch_size):
        ''' batch of trajectories '''
        # take random trajectories until we have enough
        i = 0
        indices = []
